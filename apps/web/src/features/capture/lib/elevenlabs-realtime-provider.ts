...
export class ElevenLabsRealtimeProvider implements ASRProvider {
  ...
  async initialize(
    ...
    this.emit({
      type: "ASR_STATUS",
      state: "ready",
      message: `${AI_MODELS.elevenlabs.realtimeStt} provider ready (backend proxy)`,
    });
  }
    ...
    try {
      ...
      this.emit({
        ...
        message: this.enableSystemAudio
          ? "Listening (ElevenLabs backend proxy, mic + shared audio)..."
          : "Listening (ElevenLabs backend proxy, mic only)...",
      });
    } catch (error) {
      this.stopListening();
      this.emit({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to start ElevenLabs provider",
      });
    }
  }
  ...
  private async flushAudioChunk(): Promise<void> {
    ...
    this.inFlight = true;
    try {
      const response = await fetch("/api/asr/elevenlabs/transcribe", {
        method: "POST",
      ...
      if (!response.ok) {
        this.emit({
          type: "ASR_STATUS",
          state: "processing",
          message: "ElevenLabs transcription retrying...",
        });
        return;
      }
